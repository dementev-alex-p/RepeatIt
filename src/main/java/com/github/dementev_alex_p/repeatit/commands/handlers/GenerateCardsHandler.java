...
public class GenerateCardsHandler implements CommandHandler {
    ...
    private static final String VIEW_EXAMPLES_TEXT = """
            ...
            Столицы самых популярных стран (ваш пример)
            Флаги государств мира (изображение → название страны)
            Коды валют и их символы (USD → доллар США, EUR → евро)
            Коды аэропортов IATA (JFK → Нью-Йорк, LHR → Лондон)
            ...
            Немецкий: артикли и падежи (der/die/das, Nominativ/Akkusativ)
            Японская азбука хирагана (символ → чтение)
            Английские идиомы и их русские аналоги ("It's raining cats and dogs" → "льет как из ведра")
            ...
            Регулярные выражения: самые частые шаблоны (\\d{3}-\\d{2}-\\d{4} → формат SSN)
            Команды Linux/Unix для работы с файлами (ls, cd, grep, chmod)
            
            🧠 Наука и медицина
            Периодическая таблица: символы первых 36 элементов (H → водород, O → кислород)
            ...
            Формулы для расчета площади и объема (круг: πr², куб: a³)
            Термины из психологии (когнитивный диссонанс, эффект Даннинга-Крюгера)
            
            💰 Финансы и бизнес
            Типы банковских счетов и их особенности (дебетовый, кредитный, накопительный)
            ...
            Акроны в маркетинге (AIDA, SMART, SWOT)
            
            🎨 Искусство и культура
            Знаменитые картины и их авторы ("Мона Лиза" → Леонардо да Винчи)
            Литературные персонажи и произведения (Шерлок Холмс → Конан Дойль)
            ...
            Нобелевские лауреаты (категории и открытия) (Мария Кюри — физика/химия)
            
            🍲 Кулинария и домоводство
            Пропорции для приготовления каш (рис: 1 к 2, гречка: 1 к 1,5)
            Сроки хранения продуктов в холодильнике (курица — 2 дня, яйца — 3 недели)
            Популярные специи и их применение (розмарин — к мясу, кориандр — к выпечке)
            Советы по удалению пятен (красное вино → соль + кипяток)
            
            🧘 Здоровье и фитнес
            Комплексы упражнений для утра (упражнение → название: планка, бёрпи)
            Питательная ценность продуктов (БЖУ) (куриная грудка — белок, авокадо — жиры)
            Симптомы и первая помощь (признаки инсульта — FAST, ожог — охлаждение)
            Названия мышц для тренировок (квадрицепс, бицепс бедра, дельтовидная)
            Суточные нормы витаминов и минералов (витамин C — 90 мг для мужчин)
